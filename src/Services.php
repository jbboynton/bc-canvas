...
class Services {
  public const QUERY_ARG = 'filter-by-canvas-service';
  public const TAXONOMY_ID = 'related_canvas_service';
  ...
  public function create_filters($post_type) {
    ...
    $markup .= '<option value="0">All canvas services</option>';
  ...
  public function post_states($post_states, $post) {
    if (($this->get_archive_page())->ID === $post->ID) {
      $post_states['bc_canvas_archive_page'] = CanvasPostType::PAGE_NAME;
    }
  ...
  private function on_archive_page() {
    ...
    if ($is_post_type_object) {
      ...
      $on_archive_page =
        ($queried_object->name === CanvasPostType::ID ? true : false);
  ...
  private function get_archive_page() {
    return get_page_by_path(CanvasPostType::SLUG);
  }
